...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Additional Metrics */}
      {isOnline && data.metrics && (
        <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">Additional Metrics</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {data.metrics.chunks !== undefined && data.metrics.chunks !== null && (
              <div className="text-center p-4 bg-muted/50 rounded-lg">
                <div className="text-2xl font-bold">
                  {data.metrics.chunks.toLocaleString()}
                </div>
                <div className="text-sm text-muted-foreground">Chunks Loaded</div>
              </div>
            )}
            {data.metrics.entities !== undefined && data.metrics.entities !== null && (
              <div className="text-center p-4 bg-muted/50 rounded-lg">
                <div className="text-2xl font-bold">
                  {data.metrics.entities.toLocaleString()}
                </div>
